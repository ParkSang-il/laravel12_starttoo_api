...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('business_verification_requests', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('business_verification_id')->nullable(false)->comment('사업자 인증 정보 ID (business_verifications.id)');
            $table->unsignedBigInteger('user_id')->nullable(false)->comment('사용자 ID (users.id)');
            $table->string('business_name', 100)->nullable(false)->comment('상호명 (샵 이름)');
            ...
            $table->boolean('contact_phone_public')->default(false)->nullable(false)->comment('연락처 공개 여부 (users.phone 공개 여부)');
            $table->json('available_regions')->nullable()->comment('작업 가능 지역 (JSON)');
            $table->json('main_styles')->nullable()->comment('주요 스타일 (JSON)');
            $table->string('status', 20)->default('pending')->nullable(false)->comment('승인 상태 (pending:대기중, approved:승인됨, rejected:거절됨)');
